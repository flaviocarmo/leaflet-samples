...
L.SimpleGraticule = L.LayerGroup.extend({
    ...
    buildLabel: function(axis, val) {
        ...
        var degree = Math.floor(absval);
        absval = (absval - degree) * 60;
        var minute = Math.floor(absval);
        absval = (absval - minute) * 60;
        var second = Math.floor(absval*100)/100;

        var gms = sign + ((degree < 10) ? "0" + degree : degree) + '°' + ((minute < 10) ? "0" + minute : minute) + "'" + 
              ((second < 10) ? "0" + second.toFixed(2) : second.toFixed(2)) + '"';
